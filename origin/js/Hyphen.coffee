...
class Hyphen
    PATTERNS = """
ch4 .ad4der .af1t .al3t .am5at .an5c .ang4 .ani5m .ant4 .an3te .anti5s .ar5s
...
ret-ri-bu-tion ta-blen
"""

    constructor: ()->
        @min_word = 5    # 最短长度.小于该长度将被返回.
        @min_prefix = 2  # 最小前缀长度
        @min_suffix = 2  # 最小后缀长度
        @tree = {}       # 模式树
        @exceptions = {}
    ...
    add_pattern: (pattern)->
        chars = pattern.split(//)
        chars = jQuery.grep(chars, (p)->
            return true if /\D/.test(p)
        )
        points = pattern.split(/\D/)
        points = jQuery.map(points, (p)->
            p || 0
        )
        t = @tree
        for alpha in chars
    ...
    load_pattern: ()->
        ...
        for ex in exceptions
            word = ex.replace(/-/g, '')
            @exceptions[word] = [0]
            @exceptions[word].push.apply(@exceptions[word], jQuery.map(ex.split(/[^-]/), (p)->
                if p == '-'
                    return 1
                else
                    return 0
    ...
    hyphenate: (word)->
        return [] if word.length<@min_word

        words = word.split(//)
        points = @exceptions[word.toLowerCase()]

        if not points
            work = ['.']
            work.push.apply(work,jQuery.map(words, (p)->
                p.toLowerCase()
            ))
            work.push('.')
            points = []
            for i in [0 .. work.length]
                points[i] = 0
            for i in [0 ... work.length]
                t = @tree
                for c in work.slice(i)
                    break if not t[c]
                    t = t[c]
                    p = t._

                    if p
                        points[i+j]<p[j] and points[i+j]=p[j] for j in [0 ... p.length]
            points[i] = 0 for i in [0 .. @min_prefix]
            points[i] = 0 for i in [-@min_suffix-1 .. -2]

        return points

    get_hyphened: (word)->
        points = @hyphenate(word)
        token = ''
        for i in [0 ... word.length]
            token += word[i]
            if points[i+2]%2 and i!=word.length-1
                token += '-'
        return token
